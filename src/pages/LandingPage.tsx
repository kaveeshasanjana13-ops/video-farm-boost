...
function FAQItem({ q, a, open, toggle }: { q: string; a: string; open: boolean; toggle: () => void }) {
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all duration-500 ${open ? 'bg-white border-[hsl(var(--primary)/0.2)] shadow-lg shadow-[hsl(var(--primary)/0.06)]' : 'bg-white/80 border-slate-200 hover:border-slate-300 hover:shadow-sm'}`}>
      <button onClick={toggle} className="w-full flex items-center justify-between p-5 sm:p-6 text-left gap-4">
        ...
        <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center transition-all duration-500 ${open ? 'bg-[hsl(var(--primary))] rotate-45' : 'bg-slate-100'}`}>
          <svg className={`w-4 h-4 ${open ? 'text-white' : 'text-slate-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
      ...
      <div className={`grid transition-all duration-500 ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
        <div className="overflow-hidden">
          <p className="px-5 sm:px-6 pb-5 sm:pb-6 text-sm text-slate-500 leading-relaxed">{a}</p>
        </div>
      </div>
    </div>
  );
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-800 overflow-x-hidden">
      ...
      {/* ═══════════ NAVBAR ═══════════ */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 backdrop-blur-2xl shadow-lg shadow-slate-200/50 border-b border-slate-100' : 'bg-white/60 backdrop-blur-md'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <div className="flex items-center gap-3">
              <img src={logoImg} alt="Eazy English" className="w-11 h-11 object-contain" />
              <div>
                <h1 className="text-lg sm:text-xl font-bold text-slate-900 leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Eazy English</h1>
            ...
            <div className="hidden md:flex items-center gap-1">
              {['Home', 'Process', 'Classes', 'Features', 'Gallery', 'About', 'FAQ'].map(item => (
                <a key={item} href={`#${item.toLowerCase()}`} className="relative px-3.5 py-2 rounded-xl text-sm font-medium text-slate-500 hover:text-[hsl(var(--primary))] transition-all duration-300 group">
                  {item}
            ...
            <div className="flex items-center gap-3">
              {user ? (
                ...
                <>
                  <Link to="/login" className="hidden sm:inline-flex px-5 py-2.5 rounded-xl text-sm font-semibold text-slate-600 hover:text-[hsl(var(--primary))] hover:bg-[hsl(var(--primary)/0.05)] transition-all duration-300">Sign In</Link>
              ...
              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 rounded-xl hover:bg-slate-100 transition">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white/98 backdrop-blur-xl border-t border-slate-100">
            <div className="px-4 py-4 space-y-1">
              {['Home', 'Process', 'Classes', 'Features', 'Gallery', 'About', 'FAQ'].map((item, i) => (
                <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)}
                  className="block px-4 py-3 rounded-xl text-slate-600 hover:text-[hsl(var(--primary))] hover:bg-[hsl(var(--primary)/0.05)] text-sm font-medium transition"
      ...
      <section id="home" className="relative min-h-screen flex items-center">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20 w-full">
          <div className="flex justify-center">
            <div className={`max-w-3xl text-center transition-all duration-1000 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-[hsl(var(--primary)/0.06)] backdrop-blur-md border border-[hsl(var(--primary)/0.12)] mb-8 hover:bg-[hsl(var(--primary)/0.1)] transition-colors duration-300 cursor-default">
                ...
                <span className="text-[hsl(var(--primary))] text-xs font-semibold tracking-wider">ENROLLING NOW FOR 2026</span>
              </div>
              ...
              <h2 className="text-[2.75rem] sm:text-6xl lg:text-7xl xl:text-[5.25rem] font-extrabold leading-[1.05] tracking-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                ...
                <span className="block bg-gradient-to-r from-[hsl(var(--primary))] via-[hsl(var(--primary-glow))] to-[hsl(var(--accent))] bg-clip-text animate-gradient bg-[length:200%_auto]" style={{ WebkitTextFillColor: 'transparent', WebkitBackgroundClip: 'text' }}>
                  English
                </span>
                <span className="block text-slate-700 text-[0.65em]">
                  with <span className="text-[hsl(var(--primary))]">{typewriterText}</span>
                  <span className="animate-pulse text-[hsl(var(--primary))]">|</span>
                </span>
              </h2>

              <p className="text-slate-500 text-base sm:text-lg mt-6 max-w-xl mx-auto leading-relaxed">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-10 justify-center">
                ...
                <a href="#classes" className="px-8 py-4 rounded-2xl bg-white border border-slate-200 text-slate-700 font-semibold text-base hover:bg-slate-50 hover:border-slate-300 hover:shadow-md transition-all duration-300 text-center group flex items-center justify-center gap-2">
                  Browse Classes
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2">
          <div className="flex flex-col items-center gap-2 animate-bounce-slow">
            <span className="text-slate-400 text-[10px] tracking-widest font-medium">SCROLL</span>
            ...
            <div className="grid grid-cols-3 gap-4 sm:gap-6">
              {[
                ...
                <TiltCard key={badge.title}>
                  <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-6 text-center hover:shadow-xl hover:shadow-slate-200/50 hover:border-slate-200 transition-all duration-500">
                    ...
                    <h3 className="text-sm sm:text-base font-bold text-slate-800" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{badge.title}</h3>
                    <p className="text-[11px] sm:text-xs text-slate-400 mt-1">{badge.desc}</p>
                  </div>
                </TiltCard>
              ))}
            </div>
      ...
      <section ref={statsReveal.ref} className="relative">
        <div className="absolute inset-0 bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--primary-glow))]" />
      ...
      <section className="relative h-[40vh] sm:h-[50vh] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-white via-white/40 to-white/50" />
        <div className="absolute inset-0 flex items-center justify-center">
          <Reveal direction="scale">
            <div className="text-center px-4">
              <h2 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      {/* ═══════════ PROCESS SECTION ═══════════ */}
      <section id="process" className="py-24 sm:py-32 relative bg-white">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-20">
            ...
            <h2 className="text-3xl sm:text-5xl font-extrabold text-slate-900" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
            ...
            <p className="text-slate-500 text-base sm:text-lg mt-4 max-w-2xl mx-auto">A proven 4-step process designed to take you from beginner to confident English speaker</p>
          </Reveal>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4">
            {processSteps.map((step, i) => (
              <Reveal key={step.num} delay={i * 0.12} direction={i % 2 === 0 ? 'up' : 'down'}>
                <TiltCard>
                  <GlowCard className="relative bg-white hover:bg-slate-50 border border-slate-100 hover:border-[hsl(var(--primary)/0.2)] rounded-3xl p-8 transition-all duration-500 h-full shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                    {i < 3 && <div className="hidden lg:block absolute top-12 right-0 w-full h-px bg-gradient-to-r from-slate-200 to-slate-100 translate-x-1/2 z-0" />}
                    ...
                    <h3 className="text-lg font-bold text-slate-800 mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{step.title}</h3>
      ...
      {/* ═══════════ CLASSES SECTION ═══════════ */}
      <section id="classes" className="py-24 sm:py-32 relative bg-slate-50">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-16">
            ...
            <p className="text-slate-500 text-base mt-4 max-w-xl mx-auto">Expert-crafted classes for every level — pick the one that fits your goals</p>
          </Reveal>

          {loadingClasses ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map(i => <div key={i} className="rounded-3xl h-56 skeleton border border-slate-100" />)}
            </div>
          ) : classes.length === 0 ? (
            <Reveal className="text-center py-20">
              <span className="text-5xl block mb-4">📚</span>
              <p className="text-slate-400 text-lg">Classes coming soon — stay tuned!</p>
            </Reveal>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {classes.map((cls: any, i: number) => (
                <Reveal key={cls.id} delay={i * 0.1} direction="scale">
                  <TiltCard>
                    <GlowCard className="bg-white hover:bg-white border border-slate-100 hover:border-slate-200 rounded-3xl overflow-hidden transition-all duration-500 h-full shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                      <div className={`h-1.5 bg-gradient-to-r ${gradients[i % gradients.length]}`} />
                      <div className="p-6 sm:p-7">
                        <div className="flex items-center gap-4 mb-5">
                          <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${gradients[i % gradients.length]} flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-slate-800 text-lg group-hover:text-[hsl(var(--primary))] transition-colors truncate" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{cls.name}</p>
                            {cls.subject && <p className="text-xs text-slate-400 truncate mt-0.5">{cls.subject}</p>}
                          </div>
                        </div>
                        <p className="text-sm text-slate-400 line-clamp-2 mb-5 leading-relaxed">{cls.description || 'English language class with expert instruction and interactive sessions'}</p>
                        {cls.monthlyFee != null && (
                          <div className="flex items-center justify-between pt-5 border-t border-slate-100">
                            <span className="text-xs text-slate-400 uppercase tracking-wider font-medium">Monthly</span>
      ...
      {/* ═══════════ FEATURES GRID ═══════════ */}
      <section id="features" className="py-24 sm:py-32 relative bg-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-16">
            ...
            <p className="text-slate-500 text-base mt-4 max-w-2xl mx-auto">A complete learning ecosystem powered by cutting-edge technology</p>
          </Reveal>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <Reveal key={f.title} delay={i * 0.08} direction={i < 3 ? 'left' : 'right'}>
                <TiltCard>
                  <GlowCard className="bg-white hover:bg-slate-50 border border-slate-100 hover:border-[hsl(var(--primary)/0.15)] rounded-3xl p-7 transition-all duration-500 h-full shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                    ...
                    <h3 className="text-base font-bold text-slate-800 mb-2 group-hover:text-[hsl(var(--primary))] transition-colors duration-300" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{f.title}</h3>
      ...
      {/* ═══════════ GALLERY ═══════════ */}
      <section id="gallery" className="py-24 sm:py-32 relative overflow-hidden bg-slate-50">
        <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-[hsl(var(--accent)/0.04)] rounded-full blur-[180px]" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--accent)/0.08)] border border-[hsl(var(--accent)/0.12)] mb-5">
            ...
            {galleryImages.map((img, i) => (
              <Reveal key={i} delay={i * 0.1} direction="scale">
                <div className="group relative rounded-2xl overflow-hidden aspect-[4/3] cursor-pointer shadow-md hover:shadow-xl transition-shadow duration-500">
                  ...
                  <div className="absolute inset-0 border-2 border-transparent group-hover:border-[hsl(var(--primary)/0.4)] rounded-2xl transition-all duration-500" />
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <Reveal direction="left" className="flex justify-center">
              <div className="relative group">
                ...
                <TiltCard className="relative w-72 sm:w-80 rounded-[2rem] overflow-hidden border border-slate-200 shadow-2xl shadow-slate-300/30 group-hover:border-slate-300 transition-all duration-500">
                ...
                <div className="absolute -bottom-6 -right-4 sm:right-2 lg:-right-8 bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200 p-4 shadow-2xl shadow-slate-300/30 animate-float">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-slate-900 text-lg font-black" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>95%</p>
                      <p className="text-slate-400 text-[10px] font-medium">Pass Rate</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <Reveal direction="right">
              <div className="text-center lg:text-left">
                ...
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                  Learn from the <span className="text-[hsl(var(--primary))]">Best</span>
                </h2>
                <p className="text-slate-500 text-base mt-5 leading-relaxed max-w-lg mx-auto lg:mx-0">
                  <strong className="text-slate-700">Thilina Dhananjaya</strong> is a passionate English language educator with years of experience helping students achieve outstanding results. His proven methodology combines interactive live lessons, comprehensive study materials, and personalized guidance.
                </p>
                <div className="grid grid-cols-2 gap-4 mt-10">
                  {[
                    ...
                    <Reveal key={s.label} delay={i * 0.1} direction="scale">
                      <GlowCard className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-center hover:bg-white hover:shadow-lg hover:shadow-slate-200/50 hover:border-slate-200 transition-all duration-300">
                        <span className="text-2xl block mb-2">{s.icon}</span>
                        <p className="text-xl font-black text-slate-800" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{s.num}</p>
                        <p className="text-slate-400 text-xs mt-0.5 font-medium">{s.label}</p>
                      </GlowCard>
                    </Reveal>
                  ))}
                </div>
      ...
      <section className="py-24 sm:py-32 relative bg-slate-50">
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-[hsl(var(--accent)/0.05)] rounded-full blur-[200px]" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--accent)/0.08)] border border-[hsl(var(--accent)/0.12)] mb-5">
            ...
            <p className="text-slate-500 text-base mt-4 max-w-xl mx-auto">Our students consistently achieve outstanding results across the island</p>
          </Reveal>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              { rank: '🥇', name: 'Sanduni Perera', score: 'A+', grade: '2025 O/L', gradient: 'from-amber-400 via-yellow-400 to-amber-500', shadow: 'shadow-amber-200/50' },
              { rank: '🥈', name: 'Kavindu Silva', score: 'A', grade: '2025 O/L', gradient: 'from-slate-300 via-gray-300 to-slate-400', shadow: 'shadow-gray-200/50' },
              { rank: '🥉', name: 'Nethmi Fernando', score: 'A', grade: '2025 A/L', gradient: 'from-orange-400 via-amber-500 to-orange-600', shadow: 'shadow-orange-200/50' },
              { rank: '⭐', name: 'Dilshan Kumara', score: 'A', grade: '2025 O/L', gradient: 'from-blue-400 via-indigo-400 to-blue-500', shadow: 'shadow-blue-200/50' },
            ].map((student, idx) => (
              <Reveal key={student.name} delay={idx * 0.12} direction="up">
                <TiltCard>
                  <div className="relative rounded-3xl overflow-hidden border border-slate-100 bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500">
                    ...
                    <div className="p-6 text-center">
                      ...
                      <p className="text-slate-800 font-bold text-base" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{student.name}</p>
                      <p className="text-slate-400 text-xs mt-1">{student.grade}</p>
                    </div>
                  </div>
                </TiltCard>
              </Reveal>
      ...
      <section className="py-24 sm:py-32 relative bg-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--primary)/0.06)] border border-[hsl(var(--primary)/0.1)] mb-5">
              <span className="text-[hsl(var(--primary))] text-xs font-bold tracking-wider">💬 STUDENT VOICES</span>
            </div>
            <h2 className="text-3xl sm:text-5xl font-extrabold text-slate-900" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              What Students <span className="text-[hsl(var(--primary))]">Say</span>
            </h2>
          </Reveal>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {testimonials.map((t, idx) => (
              <Reveal key={t.name} delay={idx * 0.1} direction="up">
                <TiltCard>
                  <GlowCard className="bg-white hover:bg-slate-50 border border-slate-100 hover:border-slate-200 rounded-3xl p-6 transition-all duration-500 flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                    ...
                    <p className="text-sm text-slate-500 leading-relaxed flex-1 mb-5 italic">"{t.text}"</p>
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${gradients[idx % gradients.length]} flex items-center justify-center shadow-md`}>
                        <span className="text-white text-xs font-bold">{t.avatar}</span>
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-800">{t.name}</p>
                        <p className="text-[11px] text-slate-400">{t.grade}</p>
                      </div>
                    </div>
                  </GlowCard>
                </TiltCard>
      ...
      {/* ═══════════ FAQ ═══════════ */}
      <section id="faq" className="py-24 sm:py-32 relative bg-slate-50">
        <div className="absolute top-1/4 right-0 w-96 h-96 bg-[hsl(var(--primary)/0.04)] rounded-full blur-[150px]" />
      ...
      <section className="py-20 relative bg-white">
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <Reveal key={social.platform} delay={i * 0.1} direction="up">
                <a href="#" className="group block bg-white hover:bg-slate-50 border border-slate-100 hover:border-slate-200 rounded-2xl p-6 text-center transition-all duration-500 hover:-translate-y-1 shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                  ...
                  <p className="text-slate-800 font-bold text-sm" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{social.platform}</p>
                  <p className="text-slate-400 text-xs mt-1">{social.label}</p>
                </a>
              </Reveal>
            ))}
          </div>
